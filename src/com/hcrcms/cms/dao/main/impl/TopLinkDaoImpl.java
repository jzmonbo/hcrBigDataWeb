...
@Repository
public class TopLinkDaoImpl extends HibernateBaseDao<TopLink, Integer>
		implements TopLinkDao {
	@SuppressWarnings("unchecked")
	public List<TopLink> getList() {
		Finder f = Finder.create("from TopLink bean where 1=1 ");
		f.append(" order by bean.topId");
		return find(f);
	}

	public TopLink findById(Integer id) {
		TopLink entity = get(id);
		return entity;
	}
	
	@SuppressWarnings("unchecked")
	public TopLink findByPath(String path){
		String hql = "from TopLink bean where bean.path=:path";
		List<TopLink> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	public List<TopLink> findByName(String name) {
		Finder f = Finder.create("from TopLink bean where 1=1 ");
		f.append(" and text='"+name+"' order by bean.topId");
		return find(f);
	}
	...
	@Override
	protected Class<TopLink> getEntityClass() {
		return TopLink.class;
	}
}