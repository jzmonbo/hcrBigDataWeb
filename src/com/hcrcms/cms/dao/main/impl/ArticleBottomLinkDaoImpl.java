...
@Repository
public class ArticleBottomLinkDaoImpl extends HibernateBaseDao<ArticleBottomLink, Integer>
		implements ArticleBottomLinkDao {
	@SuppressWarnings("unchecked")
	public List<ArticleBottomLink> getList() {
		Finder f = Finder.create("from ArticleBottomLink bean where 1=1 ");
		f.append(" order by bean.ablId");
		return find(f);
	}

	public ArticleBottomLink findById(Integer id) {
		ArticleBottomLink entity = get(id);
		return entity;
	}
	
	@SuppressWarnings("unchecked")
	public ArticleBottomLink findByPath(String path){
		String hql = "from ArticleBottomLink bean where bean.path=:path";
		List<ArticleBottomLink> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	public ArticleBottomLink deleteById(Integer id) {
		ArticleBottomLink entity = super.get(id);
		if (entity != null) {
	...
	public List<ArticleBottomLink> findByName(String name) {
		Finder f = Finder.create("from ArticleBottomLink bean where 1=1 ");
		f.append(" and text='"+name+"' order by bean.ablId");
		return find(f);
	}
	...
	@Override
	protected Class<ArticleBottomLink> getEntityClass() {
		return ArticleBottomLink.class;
	}
}