...
function setDateStr() {
	...
	$.get('/getactivity.htm', {
		...
		var color = ['','#6dd439','#e00000','#0e79cc'];   //颜色顺序: 无、绿、红、蓝
		var num = 0;
		datetable.find('td:not(.old,.new)').each(function(i) {					//重画活动前，先把原来的标识里"标题"和"事件"去掉
			$(this).attr('title','');
			$(this).html(i+1);
			$(this).unbind("click");
		})
		datetable.find('td:not(.old,.new)').each(function() {
			for (var i in data) {
				if (data[i]['start_time'] == parseInt($(this).text())) {
					...
					if ((parseInt(data[i]['real_start_time']) + 86400) < (Date.parse(new Date())/1000)) {
						real_color = '#ccc';																	//灰色(该事件真实时间如果小于当前时间,则颜色显示为灰色,否则显示别的颜色)
					}
					$(this).css('background', real_color);
					if($(this).attr('title')){
						num++;
						if(num > 1){
							$(this).attr('title',$(this).attr('title') + '；　活动' + (num+1) + '：' + data[i]['title']);
						}else{
							$(this).attr('title','活动' + num + '：' + $(this).attr('title') + '；　活动' + (num+1) + '：' + data[i]['title']);
						}
...
//						if(data[i]['status'] == 1){
//							if(data[i]['topic']){
//								$(this).html($(this).html() + '<a href="http://www.iyiou.com/topic/' + data[i]['name'] + '" style="color:#fff" target="_blank"></a>');
//							}else{
//								$(this).html($(this).html() + '<a href="http://www.iyiou.com/a/' + data[i]['name'] + '" style="color:#fff" target="_blank"></a>');
//							}
//							$(this).unbind("click");
//							$(this).on('click', function() {
...
//									var el = document.createElement("a");
//									document.body.appendChild(el);
//									el.href = $(this).attr('href'); //url 是你得到的连接
//									el.target = '_blank'; //指定在新窗口打开
//									el.click();
//									document.body.removeChild(el);
//								})
...
//						}
						...
						if(data[i]['status'] == 1){
							if(data[i]['topic']){
								$(this).html('<a href="http://www.iyiou.com/topic/' + data[i]['name'] + '" style="color:#fff" target="_blank">' + $(this).text() + '</a>');
							}else{
								$(this).html('<a href="http://www.iyiou.com/a/' + data[i]['name'] + '" style="color:#fff" target="_blank">' + $(this).text() + '</a>');
							}
							$(this).on('click', function() {
								var el = document.createElement("a");
								document.body.appendChild(el);
								el.href = $(this).find('a').attr('href'); //url 是你得到的连接
								el.target = '_blank'; //指定在新窗口打开
								el.click();
								document.body.removeChild(el);
							});
