...
public class ImageCutUtil {
    ...
    public static void main(String[] args) {
        try {
			ImageCutUtil is = new ImageCutUtil();
			/*try {
			    is.saveImageAsJpg("d:/imageTest/aaa.jpg", "d:/imageTest/a1.jpg", 500, 200);
			} catch (Exception e) {
			    e.printStackTrace();
			}*/
			String targetPath = "D:\\workspace\\hcrBigDataWeb\\WebContent\\m\\u\\cms\\www\\201510";
			File[] files = new File("D:\\workspace\\hcrBigDataWeb\\WebContent\\u\\cms\\www\\201510").listFiles();
			if (files.length > 0) {
				for (File file : files) {
					//System.out.println(file.getName());
					if (file.getName().contains(".jpg") || file.getName().contains(".JPG") || file.getName().contains(".png") || file.getName().contains(".PNG")
							|| file.getName().contains(".jpeg") || file.getName().contains(".JPEG") || file.getName().contains(".gif") || file.getName().contains(".GIF")){
						String houzhui = file.getName().substring(file.getName().indexOf(".")+1);
						is.saveImageAsJpg(file.getAbsolutePath(),targetPath + "/" + file.getName(), 480, 195,houzhui);
					}
				}
			}
    ...
    public void saveImageAsJpg(String fromFileStr, String saveToFileStr,
            int formatWideth, int formatHeight,String houzhui) throws Exception {
        BufferedImage srcImage;
        File saveFile = new File(saveToFileStr);
        File fromFile = new File(fromFileStr);
        //如果目标文件路径不存在,则需要创建
        String saveFilePath = saveToFileStr.substring(0,saveToFileStr.lastIndexOf("/"));
        File saveFilePathFile = new File(saveFilePath);
        if (!saveFilePathFile.exists()){
        	saveFilePathFile.mkdir();
        }
        ...
        if ("jpg".equals(houzhui) || "JPG".equals(houzhui)){
        	ImageIO.write(srcImage, "JPEG", saveFile);
        }else if ("png".equals(houzhui) || "PNG".equals(houzhui)){
        	ImageIO.write(srcImage, "png", saveFile);
        }else if ("jpeg".equals(houzhui) || "JPEG".equals(houzhui)){
        	ImageIO.write(srcImage, "JPEG", saveFile);
        }else if ("gif".equals(houzhui) || "GIF".equals(houzhui)){
