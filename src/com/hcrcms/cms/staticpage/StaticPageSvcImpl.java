...
public class StaticPageSvcImpl implements StaticPageSvc, InitializingBean {
	...
	@Transactional
	public int content(Integer siteId, Integer channelId, Date start,Date end)
			throws IOException, TemplateException {
		long time = System.currentTimeMillis();
		Map<String, Object> data = new HashMap<String, Object>();
		int count = staticPageDao.contentStatic(siteId, channelId, start,end, conf,
				data);
	...
	public void index(CmsSite site, String tpl, Map<String, Object> data)
			throws IOException, TemplateException {
		long time = System.currentTimeMillis();
		String real = getIndexPath(site);
		File f = new File(real);								//生成PC端静态文件
		File parent = f.getParentFile();
		if (!parent.exists()) {
			parent.mkdirs();
		}
		File fm = new File(MobileUtils.getMobileStaticPath(real));								//生成移动端静态文件
		parent = fm.getParentFile();
		if (!parent.exists()) {
			parent.mkdirs();
		}
		Writer out = null;
		...
		try {
			...
			outMobile = new OutputStreamWriter(new FileOutputStream(fm), UTF8);
			Template templateMobile = conf.getTemplate(MobileUtils.getMobilePath(tpl));
			templateMobile.process(data, outMobile);
