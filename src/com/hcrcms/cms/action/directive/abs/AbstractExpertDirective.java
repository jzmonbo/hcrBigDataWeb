 ...
 * 专家标签基类
 */
public abstract class AbstractExpertDirective implements
	...
	protected String getExpertId(Map<String, TemplateModel> params)
			throws TemplateException {
		return DirectiveUtils.getString(PARAM_EXPERT_ID, params);
	}
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env)
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				...
				return expertMng.getPageByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,attr, orderBy, option,
						pageNo, count,expertId,author);
			} else {
				int first = FrontUtils.getFirst(params);
				return expertMng.getListByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title, attr,orderBy, option,
						first, count,expertId,author);
			}
		}
		if (isPage()) {
			int pageNo = FrontUtils.getPageNo(env);
			return expertMng.getPageBySiteIdsForTag(siteIds, typeIds,
					titleImg, recommend, title, attr,orderBy, pageNo, count,expertId,author);
		} else {
			int first = FrontUtils.getFirst(params);
			return expertMng.getListBySiteIdsForTag(siteIds, typeIds,
					titleImg, recommend, title,attr,orderBy, first, count,expertId,author);
		}
	}
