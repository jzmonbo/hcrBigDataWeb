...
@Repository
public class ArticleTopLinkDaoImpl extends HibernateBaseDao<ArticleTopLink, Integer>
		implements ArticleTopLinkDao {
	@SuppressWarnings("unchecked")
	public List<ArticleTopLink> getList() {
		Finder f = Finder.create("from ArticleTopLink bean where 1=1 ");
		f.append(" order by bean.atlId");
		return find(f);
	}

	public ArticleTopLink findById(Integer id) {
		ArticleTopLink entity = get(id);
		return entity;
	}
	
	@SuppressWarnings("unchecked")
	public ArticleTopLink findByPath(String path){
		String hql = "from ArticleTopLink bean where bean.path=:path";
		List<ArticleTopLink> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	public ArticleTopLink deleteById(Integer id) {
		ArticleTopLink entity = super.get(id);
		if (entity != null) {
	...
	public List<ArticleTopLink> findByName(String name) {
		Finder f = Finder.create("from ArticleTopLink bean where 1=1 ");
		f.append(" and text='"+name+"' order by bean.atlId");
		return find(f);
	}
	...
	@Override
	protected Class<ArticleTopLink> getEntityClass() {
		return ArticleTopLink.class;
	}
}