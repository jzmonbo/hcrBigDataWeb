...
@Repository
public class SiteRightLinkDaoImpl extends HibernateBaseDao<SiteRightLink, Integer>
		implements SiteRightLinkDao {
	@SuppressWarnings("unchecked")
	public List<SiteRightLink> getList() {
		Finder f = Finder.create("from SiteRightLink bean where 1=1 ");
		//f.append(" order by bean.srId");
	...
	public SiteRightLink findById(Integer id) {
		SiteRightLink entity = get(id);
		return entity;
	}
	
	@SuppressWarnings("unchecked")
	public SiteRightLink findByPath(String path){
		String hql = "from SiteRightLink bean where bean.path=:path";
		List<SiteRightLink> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	public SiteRightLink deleteById(Integer id) {
		SiteRightLink entity = super.get(id);
		if (entity != null) {
	...
	public List<SiteRightLink> findByName(String name) {
		Finder f = Finder.create("from SiteRightLink bean where 1=1 ");
		f.append(" and text='"+name+"' order by bean.srId");
		return find(f);
	}
	...
	public List<SiteRightLink> findBySort(Integer start,Integer end) {
		Finder f = Finder.create("from SiteRightLink bean where bean.sort>="+start+" and bean.sort<="+end);
		Query query = getSession().createQuery(f.getOrigHql());
		List<SiteRightLink> siteRightLinks = query.list();
		return siteRightLinks;
	}

	@Override
	protected Class<SiteRightLink> getEntityClass() {
		return SiteRightLink.class;
	}
}