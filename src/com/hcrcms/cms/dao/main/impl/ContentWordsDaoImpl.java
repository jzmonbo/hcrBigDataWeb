...
@Repository
public class ContentWordsDaoImpl extends HibernateBaseDao<CmsKeyword, Integer>
		implements ContentWordsDao {
	@SuppressWarnings("unchecked")
	public List<CmsKeyword> getList() {
		Finder f = Finder.create("from CmsKeyword bean where 1=1 order by length(bean.name) desc");
		return find(f);
	}
	...
	public CmsKeyword findByPath(String path){
		String hql = "from CmsKeyword bean where bean.path=:path";
		List<CmsKeyword> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
