...
public abstract class AbstractContentDirective implements
	...
	/**
	 * 输入参数，文章内容ID，只允许输入一个，用于同类文章排队本文章
	 */
	...
	protected String[] getTagName(Map<String, TemplateModel> params){
		String[] nameStr = null;
		try {
			String tagName = DirectiveUtils.getString(PARAM_TAG_NAME, params);
			if (tagName != null && !"".equals(tagName)){
				nameStr = tagName.split(" ");
			}
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env)
		...
		String isPhone = DirectiveUtils.getString(IS_PHONE, params);
		String[] tagIds = getTagName(params);
		if (tagIds != null){
			Integer[] channelIds = getChannelIdsOrPaths(params, siteIds);
			Integer excludeId = DirectiveUtils.getInt(PARAM_EXCLUDE_ID, params);
			int first = FrontUtils.getFirst(params);
			List<Content> contents = contentMng.getListByTagIdsForTag(tagIds, siteIds,
					channelIds, typeIds, excludeId, titleImg, recommend,
					title,attr, orderBy, first, count);
			//详情页面相关文章，如果查询为空，则查询10条热门文章返回
			if (contents == null || contents.size() == 0){
				contents = contentMng.getListByChannelIdsForTag(new Integer[]{70,0},
						null, null, null, null, null, 6, 0, 1, 10,null);
				return contents;
			}else{
				return contents;
			}
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				...
				return contentMng.getPageByChannelIdsForTag(channelIds,
						...
						pageNo, count,isPhone);
			} else {
				int first = FrontUtils.getFirst(params);
				List<Content> contents =  contentMng.getListByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title, attr,orderBy, option,
						first, count,isPhone);
				if ("1".equals(isPhone)){
					Content ttContent = contentMng.findTouTiao();
					if (ttContent != null){
						ttContent.setClassify("99999");
						//ttContent.setClassify(null);
						contents.add(ttContent);
					}
				}
				//招聘文章如果为空，就把查询10条招聘文章返回
				if (contents == null || contents.size() == 0){
					String contentId = DirectiveUtils.getString(PARAM_CONTENT_ID, params);
					if (contentId != null){
						contents = contentMng.getListByChannelIdsForTag(new Integer[]{79,0},
								null, null, null, null, null,1, 0, 1, 10,null);
						return contents;
					}else{
						return contents;
					}
