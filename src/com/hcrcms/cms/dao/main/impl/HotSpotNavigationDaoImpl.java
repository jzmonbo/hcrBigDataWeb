...
@Repository
public class HotSpotNavigationDaoImpl extends HibernateBaseDao<HotSpotNavigation, Integer>
		implements HotSpotNavigationDao {
	@SuppressWarnings("unchecked")
	public List<HotSpotNavigation> getList() {
		Finder f = Finder.create("from HotSpotNavigation bean where 1=1 ");
		f.append(" order by bean.hotId");
		return find(f);
	}

	public HotSpotNavigation findById(Integer id) {
		HotSpotNavigation entity = get(id);
		return entity;
	}
	
	@SuppressWarnings("unchecked")
	public HotSpotNavigation findByPath(String path){
		String hql = "from HotSpotNavigation bean where bean.path=:path";
		List<HotSpotNavigation> list = getSession().createQuery(hql).setParameter("path", path).setMaxResults(1).list();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	public HotSpotNavigation deleteById(Integer id) {
		HotSpotNavigation entity = super.get(id);
		if (entity != null) {
	...
	public List<HotSpotNavigation> findByName(String name) {
		Finder f = Finder.create("from HotSpotNavigation bean where 1=1 ");
		f.append(" and text='"+name+"' order by bean.hotId");
		return find(f);
	}
	...
	@Override
	protected Class<HotSpotNavigation> getEntityClass() {
		return HotSpotNavigation.class;
	}
}